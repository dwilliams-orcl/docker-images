...
Oracle Real Application Clusters (Oracle RAC) is an option to the award-winning Oracle Database Enterprise Edition. Oracle RAC is a cluster database with a shared cache architecture that overcomes the limitations of traditional shared-nothing and shared-disk approaches to provide highly scalable and available database solutions for all business applications. Oracle RAC uses Oracle Clusterware as a portable cluster software that allows clustering of independent servers. These servers cooperate as a single system, and use Oracle Automatic Storage Management (Oracle ASM) to provide simplified storage management that is consistent across all servers and storage platforms. Oracle Clusterware and Oracle ASM are part of Oracle Grid Infrastructure, which bundles both solutions in an easy-to-deploy software package.
...
- [Oracle Real Application Clusters in Linux Containers](#oracle-real-application-clusters-in-linux-containers)
  ...
  - [Section 4: Oracle RAC on Docker](#section-4-oracle-rac-on-docker)
    ...
    - [Section 4.2: Set up Oracle RAC Container on Docker](#section-42-set-up-oracle-rac-container-on-docker)
    ...
    - [Section 4.3: Adding an Oracle RAC Node using a container](#section-43-adding-an-oracle-rac-node-using-a-container)
  ...
  - [Section 5: Oracle RAC on Podman](#section-5-oracle-rac-on-podman)
    ...
    - [Section 5.2: Set up Oracle RAC Containers on Podman](#section-52-set-up-oracle-rac-containers-on-podman)
    ...
    - [Section 5.3: Adding an Oracle RAC Node using a container on Podman](#section-53-adding-an-oracle-rac-node-using-a-container-on-podman)
      - [Deploying Oracle RAC on Container with Block Devices on Podman](#deploying-oracle-rac-on-container-with-block-devices-on-podman)
...
To review the resource requirements for Oracle RAC, see the Oracle Database 21c Release documentation [Oracle Grid Infrastructure Installation and Upgrade Guide](https://docs.oracle.com/en/database/oracle/oracle-database/21/cwlin/index.html)
...
4. Block storage: If you are planning to use block devices for shared storage, then allocate block devices for the Oracle Cluster Registry (OCR), voting and database files.
5. NFS storage: If you are planning to use NFS storage for OCR, voting and database files, then configure NFS storage, and export at least one NFS mount. You can also use `<GITHUM_REPO_CLONED_PATH>/docker-images/OracleDatabase/RAC/OracleRACStorageServer` container for shared file system on NFS.
...
8. To resolve VIPs and SCAN IPs, we are using a dummy DNS container in this guide. Before proceeding to the next step, create a [DNS server container](../OracleDNSServer/README.md). If you have a preconfigured DNS server in your environment, then you can replace `-e DNS_SERVERS=172.16.1.25`, `--dns=172.16.1.25`, `-e DOMAIN=example.com`  and `--dns-search=example.com` parameters in **Section 2: Building Oracle RAC Database Podman Install Images** with the `DOMAIN_NAME' and 'DNS_SERVER' based on your environment. You must ensure that you have the`Podman-docker` package installed on your Oracle Linux 8 Podman host to run the command using the `docker` utility.

9. If you are running Oracle RAC on Podman, then ensure that you have installed the `podman-docker` RPM, so that Podman commands can be run using the `docker` utility.
10. The Oracle RAC `Dockerfile` does not contain any Oracle software binaries. Download the following software from the [Oracle Technology Network](https://www.oracle.com/technetwork/database/enterprise-edition/downloads/index.html) and stage them under `<GITHUM_REPO_CLONED_PATH>/docker-images/OracleDatabase/RAC/OracleRealApplicationCluster/dockerfiles/<VERSION>` folder.
    ...
    - If you are deploying Oracle RAC on Podman, then complete this next step. Otherwise, skip to next section.
...
- To run Oracle RAC using Podman on multiple hosts, refer to [Podman `macvlan` network](https://docs.Podman.io/en/latest/markdown/Podman-network-create.1.html).
To run Oracle RAC using Oracle Container Runtime for Docker on multiple hosts, refer to [Docker `macvlan` network](https://docs.docker.com/network/macvlan/).
...
If you are planing to deploy Oracle RAC container image on Podman, then skip to the section `Oracle RAC Container Image for Podman`.
...
If you are planning to deploy the Oracle RAC container image on Docker, then skip to the section `Oracle RAC Container Image for Docker`.

 ```
 ./buildContainerImage.sh -v <Software Version> -o '--build-arg  BASE_OL_IMAGE=oraclelinux:8' -i
 #  for example ./buildContainerImage.sh -v 21.3.0
 ```
- After the `21.3.0` Oracle RAC container image is built, start building a patched image with the download 21.7 RU and one-offs. To build the patch image, see [Example of how to create a patched database image](https://github.com/oracle/docker-images/tree/main/OracleDatabase/RAC/OracleRealApplicationClusters/samples/applypatch).
...
- Edit the `/opt/.secrets/common_os_pwdfile` and seed the password for the Grid, Oracle and database users. For this deployment scenario, we use a common password. Run the command:
...
- if you want to use a common password for the Oracle, Grid, and database users, then you can assign a password file name to the environment variable COMMON_OS_PWD_FILE.
  
## Section 4: Oracle RAC on Docker

If you are deploying Oracle RAC On Podman, then skip to [Section 3: Oracle RAC on Podman](#section-3-oracle-rac-on-podman).

**Note** Oracle RAC is supported for production use on Docker starting with Oracle Database 21c (21.3). On earlier releases, Oracle RAC on Docker is supported for development and and test environments. To deploy Oracle RAC on Docker, use the pre-built images available on the Oracle Container Registry. To deploy RAC on Docker, complete all of these steps in order:
...
**IMPORTANT :** To complete configuration successfully, you must complete all prerequisite steps.
...
2. Oracle RAC must run certain processes in real-time mode. To run processes inside a container in real-time mode, you must make changes to the Docker configuration files. For details, see the [`dockerd` documentation](https://docs.docker.com/engine/reference/commandline/dockerd/#examples). Edit the Docker Daemon based on the Docker version:
   ...
   - If the Oracle `docker-engine` version is greater than or equal to 19.3: Edit `/usr/lib/systemd/system/docker.service` and add additional parameters in the `[Service]` section for the `dockerd` daemon:
...
### Section 4.2: Set up Oracle RAC Container on Docker
...
Refer the [Section 3:  Network and Password Management](#section-3--network-and-password-management) and set up the network on a container host based on your Oracle RAC environment. If you have already completed this setup, then proceed to the next step.
...
Ensure the ASM devices do not have an existing file system. To clear any other file system from the devices, use the following command:
...
Next, create the Oracle RAC container using the image. You can use the following example to create a container:
...
If you are using block devices, then skip to the section [Deploying Oracle RAC on Container with Block Devices on Docker](#deploying-oracle-rac-on-container-with-block-devices-on-docker)
...
- You must have created the `racstorage` volume before the creation of the Oracle RAC Container. For details about the available environment variables, refer to section 6.

#### Assign networks to Oracle RAC containers

You must assign the Docker networks created in section 1 to containers. To assign the networks, run the following commands:
...
If the install fails for any reason, then log in to the container using the preceding command, and check `/tmp/orod.log`. You can also review the Grid Infrastructure logs located at `$GRID_BASE/diag/crs` and check for failure logs. If the failure occurred during the database creation, then check the database logs.

### Section 4.3: Adding an Oracle RAC Node using a container

Before proceeding to the next step, ensure that Oracle Grid Infrastructure is running, and that the Oracle RAC Database is open, as instructed in [Section 4.2: Setup Oracle RAC on Docker](#section-42-setup-oracle-rac-on-docker). Otherwise, the node addition process will fail.

Refer to [Section 3:  Network and Password Management](#section-3--network-and-password-management), and set up the network on a container host based on your Oracle RAC environment. If you have already done the setup, then skip this step and continue.

To understand the details of environment variables, refer to [Section 8: Environment Variables for the Second and Subsequent Nodes](#section-8-environment-variables-for-the-second-and-subsequent-nodes)

Reset the password on the existing Oracle RAC node for SSH setup between an existing node in the cluster and the new node. The SSH password must be the same on all the nodes for the `grid` and `oracle` users. Run the following command on an existing node of the cluster:
...
**Note:** If you do not have a common secret volume among Oracle RAC containers, then populate the password file with the same password that you have used on the new node, encrypt the file, and run the script `resetOSPassword.sh` on the existing node of the cluster.

#### Deploying Oracle RAC on Container with Block Devices on Docker

If you are using an NFS volume, then skip to the section [Deploying Oracle RAC on Container with Oracle RAC Storage Container on Docker](#deploying-oracle-rac-on-container-with-oracle-rac-storage-container-on-docker-1).
...
If you are using physical block devices for shared storage, then skip to [Deploying Oracle RAC on Container with Block Devices on Docker](#deploying-oracle-rac-on-container-with-block-devices-on-docker-1).
...
- You can change environment variables such as IPs and ORACLE_PWD based on your env. For details about the env variables, refer the section 6.
...
If the node addition fails, then log in to the container using the preceding command and review `/tmp/orod.log`. You can also review the Grid Infrastructure logs (for example, `$GRID_BASE/diag/crs`) and check for failure logs. If the node creation has failed during the database creation process, then check the database logs.

## Section 5: Oracle RAC on Podman

If you are deploying Oracle RAC On Docker, then skip to [Section 2: Oracle RAC on Docker](#section-2-oracle-rac-on-docker)

**Note** Oracle RAC is supported for production use on Podman starting with Oracle Database 19c (19.16), and Oracle Database 21c (21.7). You can deploy Oracle RAC on Podman using the pre-built images available on Oracle Container Registry. To deploy Oracle RAC on Podman, Run the following steps in the order given:
...
2. Oracle RAC must run certain processes in real-time mode. To run processes inside a container in real-time mode, populate the real-time CPU budgeting on machine restarts. Use the following commands to create a oneshot systemd service:
...
### Section 5.2: Set up Oracle RAC Containers on Podman
This section provides a step-by-step procedure to deploy Oracle RAC on a container with block devices and storage container. To understand the details of environment variables, refer to [Section 7: Environment Variables for the First Node](#section-7-environment-variables-for-the-first-node)

Refer to [Section 3:  Network and Password Management](#section-3--network-and-password-management), and set up the network on a container host based on your Oracle RAC environment. If you have already completed this step, then proceed to the next step.

#### Deploying Oracle RAC Containers with Block Devices on Podman

If you are using an NFS volume, then skip to the section [Deploying Oracle RAC on Container With Oracle RAC Storage Container on Podman](#deploying-oracle-rac-on-container-with-oracle-rac-storage-container-on-podman).

Ensure that the ASM devices do not have an existing file system. To clear any other file system from the devices, use the following command:
...
Next, create the Oracle RAC container using the image. For details about environment variables, refer to section 5. You can use the following example to create a container:
...
If you are using block devices, then skip to the section [Deploying Oracle RAC Containers with Block Devices on Podman](#deploying-oracle-rac-containers-with-block-devices-on-podman).
Next, create the Oracle RAC container using the image. You can use the following example to create a container:
...
If the installation fails for any reason, then log in to the container using the preceding command, and check `/tmp/orod.log`. You can also review the Grid Infrastructure logs located at `$GRID_BASE/diag/crs`, and check for failure logs. If the failure occurred during database creation, then check the database logs.

### Section 5.3: Adding an Oracle RAC Node using a container on Podman

Before proceeding to the next step, ensure that Oracle Grid Infrastructure is running, and that the Oracle RAC Database is open, as described in [Section 5.2: Set up RAC Containers on Podman](#section-52-set-up-rac-containers-on-podman). Otherwise, the node addition process will fail.
...
#### Deploying Oracle RAC on Container with Block Devices on Podman
...
- You can change env variables such as IPs and ORACLE_PWD based on your env. For details about the environment variables, refer to section 6.
...
This section provides information about the environment variables that you can use when you create the first node of a cluster.
...
This section provides the details about the environment variables that you can use for all additional nodes added to an existing cluster.
